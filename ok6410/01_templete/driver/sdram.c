...
/*********把第bit位设置成val*********/
#define set_bit( addr, bit, val ) ( (vi addr) = (( vi addr)&=(~(0x01<<(bit))) ) | ( (val)<<(bit) ) )

/*********从第bit位开始的一共2位清零，然后把这几位设为val*********/
#define set_2bit( addr, bit, val ) ( (vi addr) = (( vi addr)&(~(0x11<<(bit))) ) | ( (val)<<(bit) ) )

/*********从第bit位开始的一共4位清零，然后把这几位设为val*********/
#define set_4bit( addr, bit, val ) ( (vi addr) = (( vi addr)&(~(0xff<<(bit))) ) | ( (val)<<(bit) ) )
...
//	set_one( P1_chip_0_cfg, 16 );		/* Bank-Row-Column organization */
	writel(0x000150f0, P1_chip_0_cfg);
	// memory init
